...
import C_Habit_class
import D_Date_class
import E_Progression_class
import F_Analysis_current_streak_per_habit
import G_Analysis_longest_streak_ever
import H_Update_progression
import J_Insert_sample_data
...
B_Table_module.check_and_create_database('Habit_app_database.db')
B_Table_module.create_habit_table('Habit_table', 'Habit_app_database.db')
B_Table_module.create_date_table('Date_table', 'Habit_app_database.db')
B_Table_module.create_progression_table('Progression_table', 'Habit_app_database.db')

# Updating the Date_table with all dates up to 'the current date' so all other functions work with an up-to-date table.
D_Date_class.Date.update_dates('Habit_app_database.db', 'Date_table')
...
def main_menu():
    ...
    if main_menu_input == "1":
        print("")
        C_Habit_class.Habit.add_habit('Habit_app_database.db')
    elif main_menu_input == "2":
        print("")
        C_Habit_class.Habit.delete_habit('Habit_app_database.db')
        ...
        if analytics_module_input == "1":
            C_Habit_class.Habit.provide_habit_list('Habit_table', 'Habit_app_database.db')
        elif analytics_module_input == "2":
            C_Habit_class.Habit.provide_habit_list_per_periodicity('Habit_table', 'Habit_app_database.db')
            ...
            E_Progression_class.Progression.combine_habits_dates('Habit_app_database.db',
                                                                 'Habit_table',
                                                                 'Date_table',
                                                                 'Progression_table')
            print("")
            print("These are all your habits including their daily/weekly streaks. ")
            F_Analysis_current_streak_per_habit.calculate_and_print_streaks('Habit_app_database.db',
                                                                            'Habit_table',
                                                                            'Progression_table')
            ...
            G_Analysis_longest_streak_ever.calculate_and_print_longest_streaks_ever(
        ...
        if reset_input == 'reset':
            I_Empty_or_delete_database.delete_tables_themselves('Habit_app_database.db',
                                                                'Habit_table',
                                                                'Date_table',
                                                                'Progression_table')
            J_Insert_sample_data.insert_sample_data_before_distribution_of_app('Habit_app_database.db',
                                                                               'Habit_table',
                                                                               'Date_table',
                                                                               'Progression_table')
            print("")
        ...
        E_Progression_class.Progression.combine_habits_dates('Habit_app_database.db',
                                                             ...
                                                             'Progression_table')
        H_Update_progression.update_function('Habit_app_database.db')
