...
# lines. See, for example, the first function in B_Table_module.
...
@patch('C_Habit_class.Habit.add_habit')  # This mocks the add_habit function. It does not actually get called.
def test_calling_add_habit(mock_add_habit, mock_input, capsys):
    ...
    with pytest.raises(SystemExit):  # The quit option in the main menu caused the test to error, this adjusts it.
        A_Habit_tracker_app.main_menu()  # Starting the main menu.
...
@patch('C_Habit_class.Habit.provide_habit_list')
...
@patch('C_Habit_class.Habit.provide_habit_list_per_periodicity')
...
@patch('F_Analysis_current_streak_per_habit.calculate_and_print_streaks')
...
@patch('G_Analysis_longest_streak_ever.calculate_and_print_longest_streaks_ever')
...
@patch('H_Update_progression.update_function')
...
@patch('I_Empty_or_delete_database.delete_tables_themselves')
@patch('J_Insert_sample_data.insert_sample_data_before_distribution_of_app')
